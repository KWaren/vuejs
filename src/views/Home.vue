<script setup lang="ts">
import { RouterLink } from 'vue-router';
import Card from '../components/Card.vue';
import { usePostIt } from "@/store/postIt";
import { useRouter } from 'vue-router';
const router = useRouter();
const store = usePostIt();
const postIts= store.postIts;;
function deleteAll() {
    store.deleteAll();
  window.location.reload();

}


</script>

<template>
     <section class="flex justify-center py-10">
            <button
                class=" flex bg-blue-600  p-3 mr-3  text-white cursor-pointer rounded-md hover:bg-blue-800 hover:scale-102 transition  duration-300 ease-in-out ">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="size-6 mr-3 bg-blue-700  rounded-sm">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                </svg>
                <RouterLink :to="{name:'notes.create'}"> Nouveau Post-It</RouterLink>
               
            </button>
            <button @click="deleteAll"
                class=" flex bg-red-100 border p-3 mx-3  text-red-600 cursor-pointer rounded-md hover:bg-red-500 hover:text-white transition  duration-300 ease-in-out ">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="size-6" >
                    <path stroke-linecap="round" stroke-linejoin="round"
                        d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                </svg>

                Supprimer tout
            </button>
        </section>
        <section class="h-full my-6 flex flex-wrap justify-center items-center">
               <RouterLink :to="{name:'notes.show',params:{_id:postIt._id}}" class="card bg-red-100 hover:shadow-xl rounded-xl hover:bg-red-200 text-gray-800 px-5 mx-[1vw] my-3 w-[90vw] sm:w-[45vw] lg:w-[31vw] h-[24vh] transition duration-300 cursor-pointer" v-for="postIt in postIts">
                <div class="flex justify-between rounded-md pt-5">
                    <h3 class="text-4xl font-bold">{{postIt.title}}</h3>
                    <RouterLink :to="{name:'notes.update',params:{ _id: postIt._id }}">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="size-8 p-2 rounded-3xl text-gray-800 bg-white transition duration-200 hover:shadow-md hover:scale-105">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                    </svg>
                    </RouterLink>
                </div>
                <p>{{postIt.content[0]}}</p>
     </RouterLink>
        </section>
</template>